export interface RuneBalance {
  rune: string
  symbol: string
  amount: number
}

interface UnisatRuneEntry {
  rune: string
  spacedRune: string
  amount: string
  divisibility: number
}

// Verified live against the real API (2026-07-09): { code, data: { detail: [...] } },
// symbol field is a single decorative glyph, so spacedRune is used as the label instead.
// start/limit are sent explicitly — the indexer endpoints page their output and
// some deployments return an empty list when the range is omitted.
export async function fetchRuneBalances(address: string, apiKey: string): Promise<RuneBalance[]> {
  if (!apiKey) throw new Error('UniSat API key не задан (Настройки)')

  const res = await fetch(
    `https://open-api.unisat.io/v1/indexer/address/${address}/runes/balance-list?start=0&limit=100`,
    { headers: { Authorization: `Bearer ${apiKey}` } },
  )
  if (res.status === 401 || res.status === 403) {
    throw new Error('UniSat: ключ отклонён — проверьте UniSat API key в Настройках')
  }
  if (res.status === 429) throw new Error('UniSat: превышен лимит запросов')
  if (!res.ok) throw new Error(`UniSat error: ${res.status}`)

  const data = await res.json()
  if (data.code !== 0) throw new Error(`UniSat: ${data.msg || 'запрос отклонён'}`)

  const payload = data.data
  const list: UnisatRuneEntry[] = Array.isArray(payload) ? payload : (payload?.detail ?? [])

  return list
    .map((r) => ({
      rune: r.rune,
      symbol: r.spacedRune || r.rune,
      amount: r.divisibility > 0 ? Number(r.amount) / 10 ** r.divisibility : Number(r.amount),
    }))
    .filter((r) => r.amount > 0)
}
